...
%
%  MATLAB's Mapping Toolbox only provides a "geotiffread" function, but it
%  does not have a "geotiffwrite" function. This is the MATLAB program to
%  write a 2D or 3D array to a single or multi-band GeoTIFF file, where data
%  can be either 1-bit monochrome data (i.e. binary or bilevel), 8-bit
%  unsigned integer, 16-bit signed / unsigned integer, 32-bit signed integer,
...
%
%          Note: "geotiffwrite" does not support full-color RGB image,
%                and the 3rd dimension only indicates different bands.
%                Therefore, when the 3rd dimension of a 3D array equals
%                to 3, it only represents a 3-band GeoTIFF image, not a
...
%
%          Note: If you set values to option.ColorMap, 16 will be
%                16-bit unsigned integer, and you cannot write in
%                32-bit depth or 1-bit depth.
%
