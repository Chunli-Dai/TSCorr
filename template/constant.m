...
flagtype=1; %image data types: 1, Landast 8; 2, Aster; 3, Planet; 4, PGC images; 5, Landsat 7 striped images; 6, subset
imageores=15; %image resolution; 15 m landsat; 4 m Planet; 2 m Wolrdview;
...
% Use the following if utm; If Arctic/Antractica, update projgdal projstrin manually. 
...
resr=imageores*4 ; %60; %30*5; % Recommend 60 m ( Old 150 m). Output spatial resolution in meters. 2; %4;%40.;
...
maxdate=9e9;%datenum('2015/10/17'); %9e9;delete data after this date
...
flageq=0; %1, earthquake, no cropping, reserved image coverage; 0, landslide, 
flagcropin=1; %1 apply subsetting (default); 0 do not apply the subsetting.
eqepoch(1)=datenum('20230205','yyyymmdd');timefix=1;
eqepoch(2)=datenum('20230206','yyyymmdd');

flagvolc=1; %duration of event time;
cosipluslist=[currentdir,'/cosipluslist.txt'];

subtiledx=10; %10 km, or 25 km, divisible by 50 km. Size of subtiles for dividing 50 km tile.
% 25 km needs 900GB and 10 CPUs; it takes 4 days for a 25 km tile.
% 10 km needs 70 GB and 10 CPUs; it takes 16 hours for a 10 km tile.
coverthres=80; % the selected images should cover the tile > 60% for landslide; > 20% for earthquake.
